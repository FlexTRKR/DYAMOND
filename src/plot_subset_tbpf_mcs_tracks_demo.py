...
def parse_cmd_args():
    ...
    parser.add_argument("--nprocesses", help="Number of processes for parallel", type=int, default=None)
...
#-----------------------------------------------------------------------
def work_for_time_loop(datafile, track_dict, map_info, figdir, figbasename=''):
    """
...
if __name__ == "__main__":
    ...
    lonv = np.arange(-180, 180.1, 20)
    latv = np.arange(-60, 60.1, 10)
    ...
    if n_workers is None: n_workers = config["nprocesses"]
    ...
    if run_parallel == 0:
        for ifile in range(len(datafiles)):
            ...
            result = work_for_time_loop(
                datafiles[ifile], track_dict, map_info, figdir, figbasename,
            )
